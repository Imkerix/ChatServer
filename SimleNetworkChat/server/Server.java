	...
	private void secredMessage(ServerClient activeclient, Msg nextCMD)
	{
		for (ServerClient soth : connectedClients)
		{
			if (soth.getNickname().equals((String) nextCMD.getObject()))
			{
				...
				if (soth != activeclient)// get a copy of the message to the sender again if you dont want to send the message to yourself
				{
					activeclient.getSt().writeMsg(activeclient.getClientsocket(), activeclient.getOut(), new Msg((String) nextCMD.getContent(), activeclient.getNickname(), 's', null));
				}
			}
		}
	}
